...
For each word, return:
      ...
      - 4 multiple choice options, including the correct meaning

      Produce JSON matching this specification:

    phrase = { "word": string, "example": string, "translation": string, "meaning": string, "options": array<string>}
...
async function fetchAndProcessPhraseData(language) {
    ...
    if (tab && tab.url) {
        ...
        try {
            const phrases = await selector.selectPhrases(currentUrl, language, 10);
